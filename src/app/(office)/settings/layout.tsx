...
import { may, type Permission } from "@/domain/permissions";
...
const TABS: { href: string; label: string; permission: Permission }[] = [
  ...
  { href: "/settings/ports", label: "Ports", permission: "app.settings" },
  { href: "/settings/filing", label: "Filing rules", permission: "app.settings" },
...
export default async function SettingsLayout({ children }: LayoutProps<"/settings">) {
  ...
  const tabs = TABS.filter((t) => may(user, t.permission));
  return (
    <>
