...
const file =
  event.tool_input?.file_path || event.tool_input?.relative_path || event.tool_input?.path || "";
if (!file || !existsSync(file)) process.exit(0);
if (!(config.lintExtensions || [".ts", ".tsx", ".js", ".jsx", ".mjs"]).includes(extname(file)))
  process.exit(0);
...
const parts = String(config.commands?.lintFile || "npx eslint --max-warnings=0")
  .split(/\s+/)
  .filter(Boolean);
