...
const ADOPTION = opt(
  "--adoption",
  existsSync("abatty.config.json") ? "abatty.config.json" : ".claude/adoption.json",
);
...
const night = {
  ADOPTION_RUN: "1",
  ADOPTION_BRANCH: "adopt/standards-selftest",
  ADOPTION_PHASE: "0",
};
...
try {
  ...
  if (settings) {
    const matchers = (event, script) =>
      (settings.hooks?.[event] || []).filter((m) =>
        (m.hooks || []).some(
          (h) =>
            (h.args || []).join(" ").includes(script) || String(h.command || "").includes(script),
        ),
      );
    ...
    check(
      "settings wires PreToolUse Edit|Write → protect.mjs",
      matchers("PreToolUse", "protect.mjs").some(
        (m) => /Edit/.test(m.matcher || "") && /Write/.test(m.matcher || ""),
      ),
      `matcher: ${
        matchers("PreToolUse", "protect.mjs")
          .map((m) => m.matcher)
          .join(", ") || "none"
      }`,
    );
    ...
    const mcpMatcher = matchers("PreToolUse", "protect.mjs").find((m) => {
      try {
        return new RegExp(m.matcher || "").test("mcp__serena__replace_content");
      } catch {
        return false;
      }
    });
    check(
      "settings wires PreToolUse mcp__* → protect.mjs",
      Boolean(mcpMatcher),
      `matcher: ${
        matchers("PreToolUse", "protect.mjs")
          .map((m) => m.matcher)
    ...
    check(
      "settings wires SessionStart → session-brief.mjs",
      wired("SessionStart", "session-brief.mjs"),
    );
    const stopTimeout = (settings.hooks?.Stop || [])
      .flatMap((m) => m.hooks || [])
      .find((h) => (h.args || []).join(" ").includes("stop-gate"))?.timeout;
    check(
      "Stop hook timeout is long enough for a gate (>= 600s)",
      (stopTimeout ?? 0) >= 600,
      `timeout=${stopTimeout}`,
    );
    ...
    check(
      "settings does not pin a config path that does not exist",
      !pinned || existsSync(pinned),
      pinned ? `env.ADOPTION_CONFIG=${pinned}` : "not pinned",
    );
    ...
    if (args.indexOf("--adoption") < 0)
      check(
        "the hooks read the config this test reads",
        resolve(configPath()) === resolve(ADOPTION),
        `hooks: ${configPath()}; here: ${ADOPTION}`,
      );
  }
  ...
  const hooksPath = (
    spawnSync("git", ["config", "--get", "core.hooksPath"], { encoding: "utf8" }).stdout || ""
  ).trim();
  if (!hooksPath)
    process.stdout.write("  ok   git hooks not installed here (npm run hooks:install)\n");
  if (hooksPath) {
    const indexed = (
      spawnSync("git", ["ls-files", "-s", "--", hooksPath], { encoding: "utf8" }).stdout || ""
    )
      .trim()
      .split("\n")
      .filter(Boolean);
    for (const line of indexed) {
      ...
      check(
        `git can run ${file}`,
        mode === "100755",
        `mode ${mode}; git update-index --chmod=+x ${file}`,
      );
    }
  }
  let adoption = null;
  try {
    ...
    check(
      "adoption names a gate command",
      typeof adoption.commands?.gate === "string" && adoption.commands.gate.length > 0,
    );
    check("adoption names the state file", typeof adoption.files?.state === "string");
    check(
      "adoption maxStopBlocks stays under the agent's own cap of 8",
      (adoption.maxStopBlocks ?? 6) < 8,
      `maxStopBlocks=${adoption.maxStopBlocks ?? 6}`,
    );
    ...
    if (existsSync(MCP_NIGHT)) {
      try {
        ...
        check(
          "mcp.night.json loads only servers adoption.json -> mcpServers names",
          undeclared.length === 0,
          `loaded: ${loaded.join(", ") || "none"}; allowed: ${allowedServers.join(", ") || "none"}`,
        );
        const unloaded = allowedServers.filter((x) => !loaded.includes(x));
        check(
          "every server adoption.json allows is loaded by mcp.night.json",
          unloaded.length === 0,
          unloaded.length ? `allowed but not loaded: ${unloaded.join(", ")}` : "",
        );
      } catch (e) {
      ...
      check(
        "mcp.night.json absent: the runner loads no MCP server, so adoption.json must allow none",
        allowedServers.length === 0,
        allowedServers.length ? `allowed: ${allowedServers.join(", ")}` : "no MCP at night",
      );
    }
    ...
    if (!pkg) {
      ...
      check(
        "gate command names an existing npm script",
        Boolean(pkg.scripts?.[m[1]]),
        `${gateCmd} -> script "${m[1]}" ${pkg.scripts?.[m[1]] ? "found" : "NOT in package.json"}`,
      );
  ...
  const cases = [
    ...
    [
      "night: push the adoption branch",
      bash("git push -u origin adopt/standards-selftest"),
      night,
      "none",
    ],
    ...
    [
      "night: push with refspec to another branch",
      bash("git push origin HEAD:release"),
      night,
      "deny",
    ],
    [
      "night: merging a pull request is a human act",
      bash("gh pr merge 12 --squash"),
      night,
      "deny",
    ],
    [
      "night: merging through the API is the same act",
      bash("gh api -X PUT repos/o/r/pulls/12/merge"),
      night,
      "deny",
    ],
    ["night: reading a pull request is not merging it", bash("gh pr view 12"), night, "none"],
    [
      "night: push -u the adoption branch with flags",
      bash("git push --set-upstream origin adopt/standards-selftest"),
      night,
      "none",
    ],
    ["night: checkout main", bash("git checkout main"), night, "deny"],
    [
      "night: checkout the adoption branch",
      bash("git checkout adopt/standards-selftest"),
      night,
      "none",
    ],
    ...
    [
      "night: pnpm install --frozen-lockfile",
    ...
    [
      "night: sed -i on a hook",
      bash("sed -i 's/exit 2/exit 0/' .claude/hooks/stop-gate.mjs"),
      night,
      "deny",
    ],
    ["night: git rm a harness file", bash("git rm .claude/settings.json"), night, "deny"],
    [
      "night: scripted write to a stop counter",
      bash("node -e \"require('fs').writeFileSync('.claude/night/stop-blocks-x.json','99')\""),
      night,
      "deny",
    ],
    [
      "night: prettier --write on the harness",
      bash("npx prettier --write .claude/hooks"),
      night,
      "deny",
    ],
    ["night: run a hook", bash("node .claude/hooks/self-test.mjs"), night, "none"],
    [
      "night: run a hook, stdout elsewhere",
      bash("node .claude/hooks/check-direction.mjs > /tmp/out.txt"),
      night,
      "none",
    ],
    ...
    [
      "night: restore the harness from the base",
      bash("git checkout main -- .claude/"),
      night,
      "none",
    ],
    [
      "night: redirect into a migration",
      bash("echo 'ALTER TABLE x' > migrations/0001_init.sql"),
      night,
      "deny",
    ],
    ...
    [
      ...
      { tool_name: "PowerShell", tool_input: { command: "git push -f origin main" } },
      {},
      "deny",
    ],
    [
      "PowerShell: Set-Content on the harness",
      {
        tool_name: "PowerShell",
        tool_input: { command: "Set-Content .claude/adoption.json '{}'" },
      },
      night,
      "deny",
    ],
  ...
  cases.push([
    "night: a corrupt adoption.json still denies a harness write",
    bash("echo x > .claude/adoption.json"),
    { ...night, ADOPTION_CONFIG: corruptCfg },
    "deny",
  ]);
  cases.push([
    "night: a corrupt adoption.json still allows an ordinary command",
    bash("npm test"),
    { ...night, ADOPTION_CONFIG: corruptCfg },
    "none",
  ]);
  ...
  cases.push([
    `the bypass flag quoted is still argv and still refused`,
    bash(`git commit "${bypass}" -m "x"`),
    {},
    "deny",
  ]);
  ...
  cases.push([
    "the short bypass flag bundled into a cluster is refused",
  ...
  cases.push([
    "a -n belonging to another command is not a bypass of this one",
    bash(`git commit -m "x" && sed -n 1p README.md`),
    {},
    "none",
  ]);
  ...
  cases.push([
    "the short force flag bundled into a cluster is refused",
    bash("git push -fu origin dev"),
    {},
    "deny",
  ]);
  cases.push(["and in the other order", bash("git push -uf origin dev"), {}, "deny"]);
  cases.push([
    "a cluster without it is ordinary work",
    bash("git push -uq origin dev"),
    {},
    "none",
  ]);
  cases.push([
    "an -f of another command is not this push's",
    bash("git push origin dev && grep -f patterns.txt src"),
    {},
    "none",
  ]);
  ...
  cases.push([
    "a push to the base by refspec is refused",
  ...
  cases.push([
    "a quoted base branch is still the base",
  ...
  cases.push([
    "a quoted refspec to the base is still the base",
    bash(`git push origin "HEAD:main"`),
    {},
    "deny",
  ]);
  cases.push([
    "a quoted branch elsewhere is still elsewhere",
  ...
  cases.push([
    "a quoted force flag is still a force push",
  ...
  cases.push([
    "a quoted forced refspec is still forced",
    bash(`git push origin "+main"`),
    {},
    "deny",
  ]);
  cases.push([
    "a quoted bypass cluster is still a bypass",
    bash(`git commit "-nm" "x"`),
    {},
    "deny",
  ]);
  cases.push([
    `a quoted long bypass flag is still one`,
    bash(`git commit "${bypass}" -m "x"`),
    {},
    "deny",
  ]);
  ...
  cases.push([
    "pushing HEAD from the base branch is a push to the base",
  ...
  cases.push([
    "pushing HEAD from another branch is not",
    bash("git push origin HEAD"),
    {},
    "none",
    offBase,
  ...
  cases.push([
    "a quoted HEAD from the base branch is still the base",
    bash(`git push origin "HEAD"`),
    {},
    "deny",
    onBase,
  ...
  cases.push([
    "a branch whose name carries the base's is not the base",
    bash("git push -u origin fix/merge-to-main-1"),
    {},
    "none",
  ]);
  cases.push([
    "nor is one that starts with it",
    bash("git push -u origin main-nav-rework"),
    {},
    "none",
  ]);
  // A redirection is the shell's, not git's: it was read as the target, and the push went through.
  cases.push([
    "a push to the base with a trailing redirection is still a push to the base",
    bash("git push origin main 2>&1"),
    {},
    "deny",
  ]);
  cases.push([
    "a push to the base into a log file is still one",
    bash("git push origin main > push.log"),
    {},
    "deny",
  ]);
  cases.push([
    "a push elsewhere with a redirection is still elsewhere",
    bash("git push -u origin feat/x 2>&1 | tail -3"),
    {},
    "none",
  ]);
  // The forge's API is another door to the same branch.
  cases.push([
    "moving the base's ref through the API is a push to the base",
    bash("gh api -X PATCH repos/o/r/git/refs/heads/main -f sha=abc123"),
    {},
    "deny",
  ]);
  cases.push([
    "merging into the base through the API is a push to the base",
    bash("gh api -X POST repos/o/r/merges -f base=main -f head=feat/x"),
    {},
    "deny",
  ]);
  cases.push([
    "reading the base's ref through the API is not a write",
    bash("gh api repos/o/r/git/refs/heads/main"),
    {},
    "none",
  ]);
  cases.push([
    "moving another ref through the API is not the base",
    bash("gh api -X PATCH repos/o/r/git/refs/heads/feat/x -f sha=abc123"),
    {},
    "none",
  ]);
  cases.push([
    `a heredoc that documents the bypass flag is a file being written`,
    bash(
  ...
  cases.push([
    "a search whose pattern names a command is a search",
    bash(`rg "git push --force" docs/`),
    {},
    "none",
  ]);
  cases.push([
    "reading the guard's own source is reading",
    bash(`grep -nE "git push|${bypass}" .claude/hooks/guard.mjs`),
    {},
    "none",
  ]);
  cases.push([
    "saying the name of a thing is not doing it",
    bash(`echo "the bypass flag is ${bypass}"`),
    {},
    "none",
  ]);
  ...
  cases.push([
    "a substitution in command position is still a force push",
    bash("$(echo git) push --force origin dev"),
    {},
    "deny",
  ]);
  cases.push([
    "so is a variable holding the program",
    bash("$GIT push --force origin dev"),
    {},
    "deny",
  ]);
  cases.push([
    "and a wrapper nobody put on a list",
    bash("timeout 5 git push --force origin dev"),
    {},
    "deny",
  ]);
  ...
  cases.push([
    "default: a commit whose message carries the authorship trailer passes (provenance kept)",
    bash(`git commit -m "feat: x\n\n${sampleTrailer()}"`),
    defaults,
    "none",
  ]);
  cases.push([
    "default, night: the same commit passes",
    bash(`git commit -m "feat: x\n\n${sampleTrailer()}"`),
    { ...night, ...defaults },
    "none",
  ]);
  cases.push([
    "scrub on, night: a commit whose message carries the authorship trailer",
    bash(`git commit -m "feat: x\n\n${sampleTrailer()}"`),
    { ...night, ...scrubOn },
    "deny",
  ]);
  cases.push([
    "scrub on, day: the same commit is refused by day too",
    bash(`git commit -m "feat: x\n\n${sampleTrailer()}"`),
    scrubOn,
    "deny",
  ]);
  cases.push([
    "scrub on, day: a pull request body that names the tool",
    bash(`gh pr create --title "feat: x" --body "${sampleTrailer()}"`),
    scrubOn,
    "deny",
  ]);
  cases.push([
    "scrub on, night: a commit without a trailer",
    bash('git commit -m "feat: x\n\nwhy it changed"'),
    { ...night, ...scrubOn },
    "none",
  ]);
  cases.push([
    "scrub on, day: a commit that names the agent's own paths only",
    bash('git commit -m "chore: restore .claude/adoption.json and CLAUDE.md from main"'),
    scrubOn,
    "none",
  ]);
  ...
  cases.push([
    "scrub on, day: a multi-line message whose only mention is the agent's own paths",
    bash(
      'git commit -m "refactor: split the gate\n\nThe step list moves out of the runner so the hook and CI read one file.\n\nRestores CLAUDE.md from main, unchanged.\n\nNo behaviour change."',
    ),
    scrubOn,
    "none",
  ]);
  cases.push([
    "scrub on, day: a multi-line message that names a tool on a later line",
    bash(
  ...
  cases.push([
    "scrub on, day: an ordinary command that names the tool is not a commit",
    bash(`echo "${sampleTrailer()}"`),
    scrubOn,
    "none",
  ]);
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...readJson(ADOPTION),
      provenance: { trailer: "Assisted-by: an unattended run" },
    }),
  );
  const provOn = { ADOPTION_CONFIG: provCfg };
  cases.push([
    "provenance, night: a commit without the disclosure trailer is refused",
    bash('git commit -m "feat: x\n\nwhy"'),
    { ...night, ...provOn },
    "deny",
  ]);
  cases.push([
    "provenance, night: a commit with the disclosure trailer passes",
    bash('git commit -m "feat: x\n\nwhy\n\nAssisted-by: an unattended run"'),
    { ...night, ...provOn },
    "none",
  ]);
  cases.push([
    "provenance, day: a human commit without the trailer is the human's decision",
    bash('git commit -m "feat: x"'),
    provOn,
    "none",
  ]);
  ...
  for (const [name, event, env, expected, cwd] of cases) {
    ...
    check(
      `guard · ${name}`,
      r.code === 0 && got === expected,
      `expected ${expected}, got ${got}${r.code !== 0 ? ", exit " + r.code : ""}`,
    );
  }
  ...
  writeFileSync(
    withMcp,
    JSON.stringify({ mcpServers: ["serena"], protectedPaths: ["migrations/", ".env"] }),
  );
  ...
  const fileCases = [
    ...
    [
      "night: edit settings.json by absolute path",
      edit(resolve(".claude/settings.json")),
      night,
      "deny",
    ],
    ...
    [
      "day: an MCP send is not judged here",
      mcp("mcp__mail_example__send_message", { channel: "x", text: "y" }),
      {},
      "none",
    ],
    [
      "night: a Slack send from the user settings",
      mcp("mcp__mail_example__send_message", { channel: "x", text: "y" }),
      night,
      "deny",
    ],
    [
      "night: a Drive file creation",
      mcp("mcp__drive_example__create_file", { name: "x" }),
      night,
      "deny",
    ],
    [
      "night: a code server not named in adoption.json",
      mcp("mcp__serena__find_symbol", { name_path: "x" }),
      night,
      "deny",
    ],
    [
      "night: a named code server, a symbol read",
      mcp("mcp__serena__find_symbol", { name_path: "x", relative_path: "src/a.ts" }),
      { ...night, ADOPTION_CONFIG: withMcp },
      "none",
    ],
    [
      "night: a named code server, an edit to a source file",
      mcp("mcp__serena__replace_content", { relative_path: "src/a.ts", needle: "a", repl: "b" }),
      { ...night, ADOPTION_CONFIG: withMcp },
      "none",
    ],
    [
      "night: a named code server, an edit to the harness",
      mcp("mcp__serena__replace_content", {
        relative_path: ".claude/adoption.json",
        needle: "a",
        repl: "b",
      }),
      { ...night, ADOPTION_CONFIG: withMcp },
    ...
    [
      "night: a named code server, an edit to an applied migration",
      mcp("mcp__serena__replace_symbol_body", {
        relative_path: "migrations/0001_init.sql",
        name_path: "x",
      }),
      { ...night, ADOPTION_CONFIG: withMcp },
      "deny",
    ...
    [
      "night: a named code server, a path outside the repository",
      mcp("mcp__serena__create_text_file", { relative_path: "../elsewhere/x.ts" }),
      { ...night, ADOPTION_CONFIG: withMcp },
      "deny",
    ],
    [
      "night: a named code server, a server name written with a dot",
      mcp("mcp__mail_example__send_message", { text: "y" }),
      { ...night, ADOPTION_CONFIG: withDotted },
      "none",
    ],
  ];
  for (const [name, event, env, expected] of fileCases) {
    ...
    check(
      `protect · ${name}`,
      r.code === 0 && got === expected,
      `expected ${expected}, got ${got}${r.code !== 0 ? ", exit " + r.code + " " + oneLine(r.stderr) : ""}`,
    );
  }
  ...
  const cfg = (gate, extra = {}) => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        commands: { gate },
        files: { state: join(tmp, "state.json") },
        maxStopBlocks: 2,
        ...extra,
      }),
    );
  ...
  const day = hook(
    "stop-gate.mjs",
    { session_id: "selftest-day" },
    { ADOPTION_CONFIG: cfg("node -e process.exit(1)") },
  );
  ...
  const red = hook(
    ...
    { session_id: sid },
    {
      ...night,
      ADOPTION_BRANCH: "",
      ADOPTION_CONFIG: cfg("node -e \"console.log('probe failure');process.exit(1)\""),
    },
  );
  check("stop-gate · red gate blocks the stop (exit 2)", red.code === 2, `exit ${red.code}`);
  check(
    "stop-gate · the reason carries the gate output",
    /gate is red/.test(red.stderr) && /probe failure/.test(red.stderr),
  );
  const redReceipt = join(nightDir, `stop-gate-${sid}.json`);
  check(
    "stop-gate · a block leaves a receipt naming the check",
    existsSync(redReceipt) &&
      readJson(redReceipt).decision === "block" &&
      readJson(redReceipt).checks.some((c) => c.check === "gate" && c.ok === false),
    existsSync(redReceipt) ? oneLine(readFileSync(redReceipt, "utf8")) : "no receipt",
  );

  const red2 = hook(
    "stop-gate.mjs",
    { session_id: sid },
    { ...night, ADOPTION_BRANCH: "", ADOPTION_CONFIG: cfg("node -e process.exit(1)") },
  );
  ...
  check(
    "stop-gate · cap reached lets the session end and says so",
    red3.code === 0 && /letting it end/.test(red3.stderr),
  );
  check(
    "stop-gate · the cap leaves a receipt",
    existsSync(redReceipt) && readJson(redReceipt).decision === "cap",
  );

  const green = hook(
    "stop-gate.mjs",
    { session_id: "selftest-green-" + Date.now() },
    {
      ...night,
      ADOPTION_BRANCH: "",
      ADOPTION_PHASE: "",
      ADOPTION_CONFIG: cfg('node -e "process.exit(0)"', { changelogRequiredFor: [] }),
    },
  );
  check(
    "stop-gate · green gate reaches the later checks (exit 0 or a named block, never a crash)",
    green.code === 0 ||
      (green.code === 2 && /Uncommitted|CHANGELOG|state|direction/.test(green.stderr)),
    `exit ${green.code} ${oneLine(green.stderr)}`,
  );
  ...
  writeFileSync(
    ...
    "﻿" +
      JSON.stringify({
        commands: { gate: 'node -e "process.exit(0)"' },
        files: { state: join(tmp, "state.json") },
  ...
  const bom = hook(
    "stop-gate.mjs",
    { session_id: "selftest-bom-" + Date.now() },
    { ...night, ADOPTION_BRANCH: "", ADOPTION_PHASE: "", ADOPTION_CONFIG: bomCfg },
  );
  check(
    "stop-gate · a config with a UTF-8 BOM is read, not a crash",
    bom.code === 0 || (bom.code === 2 && /Uncommitted|CHANGELOG|state|direction/.test(bom.stderr)),
    `exit ${bom.code}${bom.code !== 0 && bom.code !== 2 ? " " + bom.stderr.slice(0, 120) : ""}`,
  );
  ...
  const gitIn = (repo, ...a) =>
    execFileSync(
      "git",
      ["-c", "user.name=selftest", "-c", "user.email=selftest@example.invalid", ...a],
  ...
  const shape = hook(
    "stop-gate.mjs",
    { session_id: "selftest-shape-" + Date.now() },
    {
      ...night,
      ADOPTION_BRANCH: "",
      ADOPTION_PHASE: "0",
      ADOPTION_CONFIG: cfgIn("shape", {
  ...
  check(
    "stop-gate · phases that is not an array is named, not a crash",
    shape.code === 2 && /must be an array/.test(shape.stderr),
    `exit ${shape.code} ${oneLine(shape.stderr)}`,
  );

  // A phase entry not updated since the run started blocks; one updated after it lets go.
  const stale = hook(
    "stop-gate.mjs",
    { session_id: "selftest-stale-" + Date.now() },
    {
      ...night,
      ADOPTION_BRANCH: "",
      ADOPTION_PHASE: "0",
      ADOPTION_CONFIG: cfgIn("stale", {
        startedAt: "2026-01-01T00:00:00Z",
        phases: [{ id: 0, status: "pending" }],
      }),
    },
  ...
  check(
    "stop-gate · a phase entry not updated during the run blocks",
    stale.code === 2 && /no update for phase 0/.test(stale.stderr),
    `exit ${stale.code}`,
  );
  ...
  const fresh = hook(
    ...
    { session_id: freshSid },
    {
      ...
      ADOPTION_BRANCH: "",
      ADOPTION_PHASE: "0",
      ADOPTION_CONFIG: cfgIn("fresh", {
        startedAt: "2026-01-01T00:00:00Z",
        phases: [{ id: 0, status: "done", updatedAt: "2026-01-01T01:00:00Z" }],
      }),
    },
    repo,
  );
  check(
    "stop-gate · a phase entry updated during the run, clean tree, green gate: stop allowed",
    fresh.code === 0,
    `exit ${fresh.code} ${oneLine(fresh.stderr)}`,
  );
  const freshReceipt = join(nightDir, `stop-gate-${freshSid}.json`);
  check(
    "stop-gate · an allowed stop leaves a receipt with every check ok",
    existsSync(freshReceipt) &&
      readJson(freshReceipt).decision === "allow" &&
      readJson(freshReceipt).checks.every((c) => c.ok),
    existsSync(freshReceipt) ? oneLine(readFileSync(freshReceipt, "utf8")) : "no receipt",
  );
  ...
  write(
    ...
    JSON.stringify({
      commands: { gate: 'node -e "process.exit(0)"' },
      files: { state: "docs/ADOPTION_STATE.json" },
      changelogRequiredFor: [],
    }),
  );
  gitIn(trust, "add", "-A");
  ...
  write(
    ...
    ".claude/adoption.json",
    JSON.stringify({
      commands: { gate: "node -e process.exit(1)" },
      files: { state: "docs/ADOPTION_STATE.json" },
      changelogRequiredFor: [],
    }),
  );
  ...
  const trusted = hook(
    ...
    { session_id: trustSid },
    {
      ...
      ADOPTION_PHASE: "",
      ADOPTION_BASE: "main",
      ADOPTION_CONFIG: ".claude/adoption.json",
    },
    trust,
  );
  const trustReceipt = join(nightDir, `stop-gate-${trustSid}.json`);
  check(
    "stop-gate · the gate command comes from the base branch, not the edited tree",
    trusted.code === 2 &&
      !/gate is red/.test(trusted.stderr) &&
      existsSync(trustReceipt) &&
      readJson(trustReceipt).configSource === "base",
    `exit ${trusted.code}, source ${existsSync(trustReceipt) ? readJson(trustReceipt).configSource : "?"} ${oneLine(trusted.stderr)}`,
  );
  check(
    "stop-gate · an edited harness blocks with the restore instruction",
    /\.claude\/ differ from main/.test(trusted.stderr) &&
      /git checkout main -- \.claude\//.test(trusted.stderr),
    oneLine(trusted.stderr),
  );
  ...
  const corrupt = hook(
    "stop-gate.mjs",
    { session_id: "selftest-corrupt-" + Date.now() },
    {
      ...night,
      ADOPTION_PHASE: "",
      ADOPTION_BASE: "main",
  ...
  check(
    "stop-gate · a corrupt tree adoption.json is a block, never a crash",
    corrupt.code === 2 && /\.claude\//.test(corrupt.stderr),
    `exit ${corrupt.code} ${oneLine(corrupt.stderr)}`,
  );
  gitIn(trust, "checkout", "-q", "main", "--", ".claude/");
  gitIn(trust, "commit", "-q", "-m", "chore(standards): restore the harness");
  const restored = hook(
    "stop-gate.mjs",
    { session_id: "selftest-restored-" + Date.now() },
    {
      ...night,
      ADOPTION_PHASE: "",
      ADOPTION_BASE: "main",
  ...
  check(
    "stop-gate · the harness restored from the base: stop allowed",
    restored.code === 0,
    `exit ${restored.code} ${oneLine(restored.stderr)}`,
  );
  ...
  const clCfg = cfgIn(
    "changelog",
    { startedAt: "2026-01-01T00:00:00Z", phases: [] },
    {
      changelogRequiredFor: ["src/"],
      files: { state: join(tmp, "changelog-state.json"), changelog: "CHANGELOG.md" },
    },
  );
  ...
  check(
    "stop-gate · a source commit with its changelog line: stop allowed",
    covered.code === 0,
    `exit ${covered.code} ${oneLine(covered.stderr)}`,
  );
  ...
  check(
    "stop-gate · a later source commit without a changelog touch blocks and names it",
    uncovered.code === 2 &&
      /after the last CHANGELOG\.md entry/.test(uncovered.stderr) &&
      /a is 3/.test(uncovered.stderr),
    `exit ${uncovered.code} ${oneLine(uncovered.stderr)}`,
  );
  write(cl, "CHANGELOG.md", "# Changelog\n\n## [Unreleased]\n\n- a is 3\n- a is 2\n");
  gitIn(cl, "commit", "-q", "-am", "docs(changelog): a is 3");
  const cured = hook("stop-gate.mjs", { session_id: "selftest-cl3-" + Date.now() }, clEnv, cl);
  check(
    "stop-gate · a new commit with the missing entries cures it",
    cured.code === 0,
    `exit ${cured.code} ${oneLine(cured.stderr)}`,
  );
  ...
  write(
    ...
    JSON.stringify([
      ...
      { from: "src/b.ts", to: "src/a.ts", rule: { severity: "error", name: "no-circular" } },
    ]),
  );
  write(
    dir,
    ...
    JSON.stringify({
      metrics: { "size.overBudget": 5, "fn.long": 0 },
      debt: { "size.overBudget": { "src/a.ts": 5 } },
    }),
  );
  write(
    dir,
    "eslint.config.mjs",
    `export default [{ ignores: ["dist/"] }, { rules: { "max-lines": ["warn", { max: 300 }], "no-await-in-loop": "error", complexity: ["warn", 12] } }];\n`,
  );
  write(
    dir,
    "vitest.config.ts",
    `export default { test: { coverage: { thresholds: { lines: 80.5, branches: 70 } } } };\n`,
  );
  ...
  const direction = () => {
    const r = spawnSync(
      process.execPath,
      [join(HOOKS_ABS, "check-direction.mjs"), "--base", "main"],
      { cwd: dir, encoding: "utf8", env: { ...process.env, ...baseEnv } },
    );
  ...
  const expectDirection = (name, mutate, expectCode, pattern) => {
    ...
    check(
      `direction · ${name}`,
      r.code === expectCode && (!pattern || pattern.test(r.out)),
  ...
  expectDirection(
    "a baseline number that rose blocks",
    () =>
      write(
        dir,
        "scripts/ci/standards-baseline.json",
        JSON.stringify({
          metrics: { "size.overBudget": 6, "fn.long": 0 },
          debt: { "size.overBudget": { "src/a.ts": 6 } },
        }),
      ),
    2,
    /rose 5 → 6/,
  ...
  expectDirection(
    "a baseline number that fell is clean",
    () =>
      write(
        dir,
        "scripts/ci/standards-baseline.json",
        JSON.stringify({
          metrics: { "size.overBudget": 4, "fn.long": 0 },
          debt: { "size.overBudget": { "src/a.ts": 4 } },
        }),
      ),
    0,
  );
  expectDirection(
    "a metric removed from the baseline blocks",
    () =>
      write(
        dir,
        "scripts/ci/standards-baseline.json",
        JSON.stringify({ metrics: { "size.overBudget": 5 }, debt: {} }),
      ),
    2,
    /fn\.long.*gone/,
  );
  ...
  expectDirection(
    "a rule switched off without a decision blocks",
    () => write(dir, "eslint.config.mjs", ruleOff),
    2,
    /no-await-in-loop/,
  );
  expectDirection(
    "the same rule off with a decision naming it is recorded, not refused",
    () => {
      write(dir, "eslint.config.mjs", ruleOff);
      write(
        dir,
    ...
    0,
    /with a decision naming it/,
  );
  expectDirection(
    "a path added to ignores blocks",
    () =>
      write(
        dir,
        "eslint.config.mjs",
        `export default [{ ignores: ["dist/", "src/legacy/"] }, { rules: { "max-lines": ["warn", { max: 300 }], "no-await-in-loop": "error", complexity: ["warn", 12] } }];\n`,
      ),
    2,
    /src\/legacy\/.*added to ignores/,
  );
  expectDirection(
    "a rule tightened and a new block at error are clean",
    () =>
      write(
        dir,
        "eslint.config.mjs",
        `export default [{ ignores: ["dist/"] }, { rules: { "max-lines": ["error", { max: 300 }], "no-await-in-loop": "error", complexity: ["warn", 12] } }, { files: ["src/**"], rules: { "no-console": "error" } }];\n`,
      ),
    0,
  );
  expectDirection(
    "a coverage threshold that fell blocks",
    () =>
      write(
        dir,
        "vitest.config.ts",
        `export default { test: { coverage: { thresholds: { lines: 70, branches: 70 } } } };\n`,
      ),
    2,
    /lines threshold #1 fell 80\.5 → 70/,
  );
  expectDirection(
    "thresholds.autoUpdate appearing blocks",
    () =>
      write(
        dir,
        "vitest.config.ts",
        `export default { test: { coverage: { thresholds: { lines: 80.5, branches: 70, autoUpdate: true } } } };\n`,
      ),
    2,
    /autoUpdate/,
  );
  expectDirection(
    "--max-warnings=0 dropped from the lint script blocks",
    () => write(dir, "package.json", pkgWith({ lint: "eslint ." })),
    2,
    /no longer runs with --max-warnings=0/,
  );
  expectDirection(
    "the gate script removed from package.json blocks",
    () =>
      write(
        dir,
        "package.json",
        JSON.stringify({ scripts: { ...basePkg.scripts, gate: undefined } }),
      ),
    2,
    /"gate" was removed/,
  );
  ...
  expectDirection(
    "known import-graph violations that grew block",
    () =>
      write(
        dir,
        ".dependency-cruiser-known-violations.json",
        JSON.stringify([
          { from: "src/a.ts", to: "src/b.ts", rule: { severity: "error", name: "no-circular" } },
          { from: "src/b.ts", to: "src/a.ts", rule: { severity: "error", name: "no-circular" } },
          { from: "src/c.ts", to: "src/d.ts", rule: { severity: "error", name: "no-orphans" } },
        ]),
      ),
    2,
    /known import-graph violations rose 2 → 3/,
  );
  expectDirection(
    "known import-graph violations that shrank are clean",
    () =>
      write(
        dir,
        ".dependency-cruiser-known-violations.json",
  ...
  expectDirection(
    "the known-violations file deleted blocks",
    () => rmSync(join(dir, ".dependency-cruiser-known-violations.json")),
    2,
    /known-violations file of the import graph was deleted/,
  );
  expectDirection(
    "knip --max-issues raised blocks",
    () => write(dir, "package.json", pkgWith({ dead: "knip --max-issues 9" })),
    2,
    /knip --max-issues rose 3 → 9/,
  );
  expectDirection(
    "knip --max-issues lowered is clean",
    () => write(dir, "package.json", pkgWith({ dead: "knip --max-issues 0" })),
    0,
  );
  expectDirection(
    "knip gaining --no-exit-code blocks",
    () => write(dir, "package.json", pkgWith({ dead: "knip --max-issues 3 --no-exit-code" })),
    2,
    /gained --no-exit-code/,
  );
  expectDirection(
    "depcruise losing --output-type err blocks",
    () =>
      write(
        dir,
        "package.json",
        pkgWith({ graph: "depcruise src --config .dependency-cruiser.cjs --ignore-known" }),
      ),
    2,
    /--output-type err dropped/,
  );
  expectDirection(
    "a strict flag turned off blocks",
    () => write(dir, "tsconfig.json", `{ "compilerOptions": { "strict": false } }\n`),
    2,
    /"strict" was true/,
  );
  expectDirection(
    "a pre-push hook that no longer runs the gate blocks",
    () => write(dir, ".githooks/pre-push", "#!/bin/sh\nexit 0\n"),
    2,
    /no longer mentions the gate/,
  );
  expectDirection(
    "a CI step made non-blocking blocks",
    () =>
      write(
        dir,
  ...
  expectDirection(
    "a harness file added to the index blocks",
    () => {
      write(dir, ".claude/adoption.json", "{}");
      gitIn(dir, "add", ".claude/adoption.json");
    },
    2,
    /under \.claude\/ differ from main/,
  );
  gitIn(dir, "rm", "-q", "--cached", ".claude/adoption.json");
  rmSync(join(dir, ".claude"), { recursive: true, force: true });
  const unknownBase = spawnSync(
    process.execPath,
    [join(HOOKS_ABS, "check-direction.mjs"), "--base", "no-such-branch"],
    { cwd: dir, encoding: "utf8", env: { ...process.env, ...baseEnv } },
  );
  check(
    "direction · an unknown base is reported, not judged (exit 1)",
    unknownBase.status === 1 && /not found/.test(unknownBase.stdout + unknownBase.stderr),
    `exit ${unknownBase.status}`,
  );
} finally {
...
try {
  ...
  if (!scrubOnHere) {
    check(
      "user settings: attribution left to the agent (provenance is the default; scrub.enabled is off)",
      true,
    );
    throw null;
  }
  ...
  check(
    "user settings: attribution.commit is empty (no trailer on the night's commits)",
    commitAttr === "",
    us
      ? commitAttr === undefined
        ? "attribution not set: merge the attribution block of templates/harness/settings.user.json into ~/.claude/settings.json"
        : `attribution.commit=${JSON.stringify(commitAttr)}`
...
try {
  ...
  if (!agent)
    throw new Error("no agent command: set ABATTY_AGENT or agent.command in ~/.abatty/config.json");
  ...
  const v = execFileSync(batch ? `"${exe}"` : exe, ["--version"], {
    encoding: "utf8",
    shell: batch,
  }).trim();
  ...
  const ok =
    m &&
    (Number(m[1]) > 2 ||
      (Number(m[1]) === 2 && (Number(m[2]) > 1 || (Number(m[2]) === 1 && Number(m[3]) >= 259))));
