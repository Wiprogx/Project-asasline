...
process.on("uncaughtException", (err) => {
  process.stderr.write(
    `[stop-gate] internal error, refusing the stop: ${err && err.stack ? err.stack : err}\n`,
  );
  process.exit(2);
});
...
if (blocks.value >= (config.maxStopBlocks || 6)) {
  ...
  process.stderr.write(
    `[stop-gate] ${blocks.value} blocks this session; letting it end. The runner will mark the phase blocked.\n`,
  );
  process.exit(0);
}
...
const pass = (check, detail) =>
  receipt.checks.push({ check, ok: true, ...(detail ? { detail } : {}) });
...
if (wanted && branch !== wanted) {
  block(
    "branch",
    `You are on "${branch}" but the run works on "${wanted}". Run: git checkout ${wanted}`,
  );
}
pass("branch");
...
if (source === "tree")
  process.stderr.write(
    `[stop-gate] adoption.json is not on ${base}; using the working-tree copy\n`,
  );
try {
  execSync(gateCmd, {
  ...
  block(
    "gate",
    `The gate is red (${gateCmd}). Fix the cause, never the threshold; never add an eslint-disable or raise a baseline number to pass. Last lines:\n${tail(out, 80)}`,
  );
}
pass("gate", gateCmd);
...
if (mergeBase) {
  ...
  const offenders = [];
  for (const sha of git("log", "--format=%H", "--no-merges", `${mergeBase}..HEAD`)
    .split("\n")
    .filter(Boolean)) {
...
if (
  ...
  try {
    ...
    if (!Array.isArray(state.phases)) {
      block(
        "state",
        `${stateFile}: "phases" must be an array of { id, status, ... }, got ${state.phases === undefined ? "nothing" : typeof state.phases}. Rewrite it as an array (one entry per phase in .claude/adoption.json) and commit.`,
      );
    }
    ...
    if (!entry || !run.startedAt || updated < run.startedAt) {
      block(
        "state",
        `${stateFile} has no update for phase ${phase} since the run started (${run.startedAt || "unknown"}). Set phases[${phase}].status to done | in_progress | blocked with reason, numbersBefore, numbersAfter, updatedAt (ISO), and commit it.`,
      );
    }
  } catch (err) {
    block(
      "state",
      `${stateFile} is unreadable: ${err.message}. Restore a valid JSON state file and commit it.`,
    );
  }
}
pass("state");
