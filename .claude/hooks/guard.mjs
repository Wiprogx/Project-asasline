...
const invocations = (name) =>
  segments.filter((s) => !s.opaque && s.program === name).map((s) => s.args);
...
function deny(reason) {
  if (NIGHT)
    appendLog("guard-denials", { tool: event.tool_name, command: cmd.slice(0, 300), reason });
  decide("deny", reason);
  process.exit(0);
}
...
const forces = (a) =>
  carries(a, "f", "--force", "--force-with-lease") ||
  a.some((x) => /^\+[\w/]/.test(x) && !x.startsWith("-"));
const forcePush =
...
if (
  forcePush ||
  hasFlag(
    /\bgit push\b[^;&|]*(\s--force\b|\s--force-with-lease\b|\s-[a-zA-Z]*f[a-zA-Z]*\b|\s\+[\w/])/,
  )
) {
...
const bypass =
  invocations("git").some(
    (a) => a.includes("--no-verify") || (sub(a) === "commit" && carries(a, "n")),
...
const isCommitText =
  /\bgit commit\b|\bgh pr (create|edit|merge)\b|\bgit tag\b.*-m|\bgh (issue|release) create\b/.test(
    cmd,
  );
if (config.scrub?.enabled === true && isCommitText) {
  ...
  if (said)
    deny(
      `No trace of the tools in a commit, a tag, a pull request or an issue (scrub.enabled): the text names one (${said.trim().slice(0, 80)}). Say it again without the name.`,
    );
}
...
const trailer =
  typeof config.provenance?.trailer === "string" ? config.provenance.trailer.trim() : "";
if (NIGHT && trailer && /\bgit commit\b.*\s-m\b/.test(cmd) && !cmd.includes(trailer)) {
  deny(
    `Unattended commits carry the disclosure trailer this repository asks for (provenance.trailer): add "${trailer}" as the last line of the message.`,
  );
}
...
function destinationOf(args) {
  ...
  const dest = (spec.includes(":") ? spec.slice(spec.lastIndexOf(":") + 1) : spec).replace(
    /^refs\/heads\//,
    "",
  );
...
const targets = [
  ...
  ...maybeGit
    .filter((t) => t.includes("push"))
    .map((t) => destinationOf(t.slice(t.indexOf("push")))),
...
const apiRefWritePrecise = invocations("gh").some((a) => {
  ...
  const writes =
    (method >= 0 && WRITE_METHOD.test(a[method + 1] || "")) || a.some((x) => BODY_FLAG.test(x));
  const path = a.find((x) => x.includes("/") && !x.startsWith("-")) || "";
  return writes && new RegExp(`/(git/refs/heads/(${base}|master)\\b|merges\\b)`).test(path);
});
const apiWrite =
  /\bgh api\b/.test(opaqueText) &&
  /\s(-X|--method)\s+(PATCH|POST|PUT|DELETE)\b|\s(-f|-F|--field|--raw-field|--input)\b/i.test(
    opaqueText,
  );
...
const apiRefWrite =
  ...
  (apiWrite &&
    new RegExp(
      B + "gh api" + B + "[^;&|]*/(git/refs/heads/(" + base + "|master)" + B + "|merges" + B + ")",
    ).test(opaqueText));
...
if (NIGHT) {
  ...
  if (has(/\bgit push\b/)) {
    ...
    if (!adoption || pushed !== adoption) {
      deny(
        `Unattended run: pushes go to ${adoption || "the adoption branch"} only (this one targets "${pushed}").`,
      );
    }
  }
  ...
  if (leaves) {
    deny(
      `Unattended run: stay on ${adoption || "the adoption branch"}. Switching branches is not allowed.`,
    );
  }
  const rules = [
    [
      /\bgh pr merge\b|\bgh api\b[^;&|]*\/pulls\/\d+\/merge\b/,
      "Merging a pull request is a human act; the morning reads the branch.",
    ],
    ...
    [
      /\bgit branch\s+(-D|--delete --force)\b/,
      "Force-deleting a branch is not allowed unattended.",
    ],
    [
      /\bgit (rebase|commit --amend|filter-branch|reflog expire)\b/,
      "History rewriting is not allowed unattended.",
    ],
    [
      /\brm\s+-[a-zA-Z]*r[a-zA-Z]*\s+(\/|~|\.\.|\.git\b|\*|\$HOME)/,
      "Recursive delete outside or at the root of the tree is not allowed.",
    ],
    [
      /\bRemove-Item\b.*-Recurse\b.*(\\\\|\/|~|\.\.|\.git\b|\*)/,
      "Recursive delete outside or at the root of the tree is not allowed.",
    ],
    [
      /\b(DROP\s+(DATABASE|TABLE|SCHEMA|ROLE)|TRUNCATE\s+(TABLE\s+)?\w)/i,
      "Destructive SQL is not allowed unattended.",
    ],
    ...
    [
      /\b(coolify|deploy|release)\b.*\.(sh|ps1)\b|\bdocker compose\b.*production|\bproduction-deploy\b/i,
      "Deploying is not allowed unattended.",
    ],
    ...
    [
      /\b(npm|pnpm|yarn)\s+(install|add|i|update|upgrade|remove|uninstall)\s+(?!-)\S/,
      "Dependency changes are deferred to the morning (decision: dependency-deferred). Record the need in docs/ADOPTION_DECISIONS.md.",
    ],
    [
      /\b(sequelize|prisma|drizzle-kit)\b.*\b(migrate|push)\b.*(production|testing|test\.env|\.env\.prod)/i,
      "A migration against a non-local database is not allowed.",
    ],
  ];
...
const warnings = [
  ...
  [
    /\b(DROP\s+(DATABASE|TABLE|SCHEMA)|TRUNCATE\b|DELETE\s+FROM)/i,
    "Destructive SQL: check the target database.",
  ],
];
...
function writesTo(command, p) {
  ...
  const verb =
    /(^|\s)(rm|mv|cp|tee|truncate|touch|patch|sed\s+-i\S*|git\s+(rm|mv|apply)|Remove-Item|Move-Item|Copy-Item|Set-Content|Add-Content|Out-File|Clear-Content|New-Item|(npx\s+)?prettier\s+(--write|-w)|(npx\s+)?eslint\s+.*--fix)\b/;
  const redirect = new RegExp(`>{1,2}\\s*["']?[^\\s"'|;&]*${esc}`);
  const scripted = new RegExp(
    `(writeFile|writeFileSync|appendFile|appendFileSync|WriteAllText|AppendAllText|open\\([^)]*['"][wa])[^;]*${esc}`,
  );
  return command
    .split(/\s*(?:\|\||&&|;|\|)\s*/)
    .some(
      (seg) => (seg.includes(p) && (verb.test(seg) || scripted.test(seg))) || redirect.test(seg),
    );
}
